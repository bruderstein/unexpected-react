...
describe('unexpected-react-shallow', function () {
    ...
    describe('diff', function () {
        ...
        it('matches className with an extra class on actual', function () {
            renderer.render(React.createElement(MyDiv, { className: 'two one four' }));

            return expect(renderer, 'to have rendered', React.createElement('div', { className: 'one two' }));
        });

        it('diffs className with an extra class on actual with `with all classes', function () {
            renderer.render(React.createElement(MyDiv, { className: 'two one four' }));

            expect(function () {
                return expect(renderer, 'to have rendered with all classes', React.createElement('div', { className: 'one two' }));
            }, 'to throw', ['expected <div className="two one four" />', 'to have rendered with all classes <div className="one two" />', '', '<div className="two one four" // extra class \'four\'', '/>'].join('\n'));
        });
    ...
    describe('when rendered', function () {
        ...
        it('renders a class component with the shallow renderer', function () {
            expect(React.createElement(ClassComponent, null), 'when rendered', 'to have rendered', React.createElement('div', { className: 'class-component' }));
        });

        it('passes the renderer on as the fulfillment of the promise', function () {
            return expect(React.createElement(ClassComponent, null), 'when rendered').then(function (renderer) {
                expect(renderer, 'to be a', 'ReactShallowRenderer');
                // Just an extra check, just in case the thing only looks like a renderer :)
                expect(renderer.getRenderOutput(), 'to equal', React.createElement('div', { className: 'class-component' }));
            });
        });

        it('renders a stateless component', function () {
            expect(React.createElement(FunctionComp, { className: 'test' }), 'when rendered', 'to have rendered', React.createElement('div', { className: 'test' }));
        });

        it('passed the error on when the following assertion fails', function () {
            expect(function () {
                return expect(React.createElement(FunctionComp, { className: 'test' }), 'when rendered', 'to have rendered', React.createElement('div', { className: 'testx' }));
            }, 'to throw', ['expected <FunctionComp className="test" /> when rendered', 'expected <div className="test" /> to have rendered <div className="testx" />', '', '<div className="test" // missing class \'testx\'', '/>'].join('\n'));
        });
    });

    describe('to render as', function () {
        it('validates a render', function () {
            expect(React.createElement(FunctionComp, { className: 'test' }), 'to render as', React.createElement('div', { className: 'test' }));
        });

        it('outputs an error when it fails', function () {
            expect(function () {
                return expect(React.createElement(FunctionComp, { className: 'foo' }), 'to render as', React.createElement('div', { className: 'bar' }));
            }, 'to throw', ['expected <FunctionComp className="foo" /> to render as <div className="bar" />', '', '<div className="foo" // missing class \'bar\'', '/>'].join('\n'));
        });

        it('uses the exactly flag', function () {
            expect(function () {
                return expect(React.createElement(FunctionComp, { className: 'foo' }), 'to exactly render as', React.createElement('div', null));
            }, 'to throw', ['expected <FunctionComp className="foo" /> to exactly render as <div />', '', '<div className="foo" // className should be removed', '/>'].join('\n'));
        });

        it('uses the `with all classes` flag', function () {

            expect(function () {
                return expect(React.createElement(FunctionComp, { className: 'foo bar' }), 'to render with all classes as', React.createElement('div', { className: 'foo' }));
            }, 'to throw', ['expected <FunctionComp className="foo bar" />', 'to render with all classes as <div className="foo" />', '', '<div className="foo bar" // extra class \'bar\'', '/>'].join('\n'));
        });
    });
});